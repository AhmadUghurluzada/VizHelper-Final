...
def _apply_colorblind_palette(ax, config):
    """
    Apply a colorblind-friendly palette only when user_profile == 'colorblind':
    - For bar charts: override only low-contrast original bar colors (e.g., green/red) to a single high-contrast color.
    - For other chart types: cycle through palette.
    """
    palette = config.get("color_palette", [])
    chart_type = _detect_chart_type(ax)
    # Only override when colorblind profile
    if chart_type == "bar" and palette:
        high_contrast = palette[0]
        low_contrast_hex = {"#4daf4a", "#e41a1c", "#999999"}
        import matplotlib.colors as mcolors
        for bar in ax.patches:
            if isinstance(bar, mpl.patches.Rectangle) and bar.get_width() > 0:
                orig = mcolors.to_hex(bar.get_facecolor())
                if orig in low_contrast_hex:
                    bar.set_facecolor(high_contrast)
        return
    # Cycle through palette otherwise
    idx = 0
    for bar in ax.patches:
        if isinstance(bar, mpl.patches.Rectangle) and bar.get_width() > 0:
            bar.set_facecolor(palette[idx % len(palette)])
            idx += 1
